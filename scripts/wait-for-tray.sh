...
# until waybar's tray module has actually initialized - not just started.
#
...
# never retry. Waybar takes a moment after exec to publish
# org.kde.StatusNotifierWatcher, so any tray app launched in the same
# breath as waybar in autostart.lua is racing it - most visibly Mullvad's
...
# Watcher never showed up (waybar crashed, tray module disabled, etc.) -
