...
#
#   sudo ./system/iso/build.sh            build into system/iso/out/
#   sudo ./system/iso/build.sh /mnt/big    build with the work directory elsewhere
#
...
# system/install/packages.txt long after this, so they do not belong here.
...
# git clone rather than cp or git archive: a clone reads committed objects
# only, so the ISO still can never carry a stray local edit or an untracked
# file - but unlike archive it keeps .git, so dotup works on the installed
# system without a re-clone. --no-hardlinks so the ISO build never shares
# inodes with this repo's own object store.
