...
(use-package! presence
  :config
  (setq presence-refresh-rate 15)
  (setq presence-idle-timer 300)
  (setq presence-idle-message "Away from keyboard")
  (setq presence-quiet t)
  ...
  (setq presence-buffer-details-format-function
        (lambda ()
          (condition-case nil
              (let* ((buffer-name (or (buffer-name) "Unknown"))
                     (project-name
                      (cond
                       ((and (fboundp 'doom-project-name)
                             (doom-project-name))
                        (doom-project-name))
                          ...
                          (file-name-nondirectory
                           (directory-file-name
                            (if (fboundp 'project-root)
                              ...
                              (cdr project))))))
                       (t nil)))
                     (line-info (when (and presence-display-line-numbers
                                           (not (minibufferp)))
                                  (format ":%d" (line-number-at-pos)))))
                (cond
                 ((and project-name line-info)
                  (format "%s | %s%s" project-name buffer-name line-info))
                 (project-name
                  (format "%s | %s" project-name buffer-name))
                 (line-info
                  (format "%s%s" buffer-name line-info))
                 (t buffer-name)))
