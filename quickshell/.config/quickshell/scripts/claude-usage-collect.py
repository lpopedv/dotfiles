...
Vendored from Omarchy (bin/omarchy-agent-usage-claude, MIT, Copyright (c)
David Heinemeier Hansson), unchanged but for this paragraph. Refresh it from
https://github.com/basecamp/omarchy (branch quattro); do not edit in place.

Everything shown for Claude comes from this one command: local transcript
stats from ~/.claude/projects, the stats-cache and history fallbacks for
machines without transcripts, pi/omp and opencode sessions that ran on an
Anthropic provider, and the authoritative rate limits from Anthropic's OAuth
usage endpoint. Callers only ever read the JSON this prints; they never talk
to disk formats or endpoints.

Usage: claude-usage-collect.py [--force] [--limits-only]
"""
